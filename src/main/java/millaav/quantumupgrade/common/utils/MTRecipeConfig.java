...
public class MTRecipeConfig {
    ...
    public static String formatItemData(millaav.quantumupgrade.common.utils.MTRecipeManager.RawItemData rawItemData) {
    ...
    public static void addDefaultRecipe(Object input, int inputStackSize, Object output, int outputStackSize, int energy) {
        ...
        if (input instanceof String) {
            ...
            result = result + formatItemData(millaav.quantumupgrade.common.utils.MTRecipeManager.getItemData((ItemStack)input)) + "-" + ((ItemStack)input).getItemDamage() + ":" + ((ItemStack)input).stackSize + "; ";
        }
    ...
    public static List<millaav.quantumupgrade.common.utils.MTRecipeRecord> parse(File f) {
        List<millaav.quantumupgrade.common.utils.MTRecipeRecord> list = Lists.newArrayList();

        try {
            if (!f.exists()) {
                QuantumUpgrade.addLog("MT recipes config file not found. Create default recipes.");
                f.createNewFile();
            ...
            QuantumUpgrade.addLog("* * * * * * Start parsing MT recipes * * * * * * ");
                ...
                while(true) {
                    do {
                        if ((line = br.readLine()) == null) {
                            br.close();
                            fis.close();
                            QuantumUpgrade.addLog("* * * * * * Finished parsing MT recipes * * * * * * ");
                            QuantumUpgrade.addLog(String.format("Config loading completed, %d recipes parsed.", list.size()));
                            QuantumUpgrade.addLog("* * * * * * Loaded recipes list * * * * * * ");
                            Iterator i$ = list.iterator();

                            while(i$.hasNext()) {
                                millaav.quantumupgrade.common.utils.MTRecipeRecord record = (millaav.quantumupgrade.common.utils.MTRecipeRecord)i$.next();
                                ...
                                result = result + (output instanceof ItemStack ? ((ItemStack)output).getDisplayName() : (String)output);
                                result = result + String.format(" [%d EU]", energy);
                                QuantumUpgrade.addLog(result);
                            }

                            QuantumUpgrade.addLog("* * * * * * * * * * * * * * * * * * * * *");
                            return list;
                        }
                    ...
                    if (spaceSplit.length != 3) {
                        QuantumUpgrade.addLog(String.format("Ignoring line %d, incorrect format.", lineNumber));
                    } else {
                        ...
                        try {
                            ...
                            QuantumUpgrade.addLog(String.format("Ignoring line %d, energy number is incorrect.", lineNumber));
                            continue;
                        }

                        List<ItemStack> inputStacks = wrap(input);
                        if (inputStacks == null) {
                            QuantumUpgrade.addLog(String.format("Ignoring line %d, failed to parse input.", lineNumber));
                        } else {
                            List<ItemStack> outputStacks = wrap(output);
                            if (outputStacks == null) {
                                QuantumUpgrade.addLog(String.format("Ignoring line %d, failed to parse output.", lineNumber));
                            } else {
            ...
            Throwable t = var17;
            QuantumUpgrade.addLog("Fatal error occurred parsing MT recipes config. (" + t.toString() + ")");
            return list;
        }
    }
