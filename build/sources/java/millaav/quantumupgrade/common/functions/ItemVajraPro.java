...
public class ItemVajraPro extends ItemTool implements IElectricItem {

    public int maxCharge, transferLimit, tier, epo;
    public float effPower;
    int attackDamage = 3;

    public ItemVajraPro(float p_i45333_1_, ToolMaterial p_i45333_2_, Set p_i45333_3_) {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int p_77663_4_, boolean p_77663_5_) {
        ...
        if(ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.RANGE)){tag.setFloat("attackradius",5.0F);}else {tag.setFloat("attackradius",1.0F);}
        if(ItemVajraPro.hasUpgrade(stack,EnumUpgradeType.SILKTOUCH)){tag.setBoolean("silktouch", true);}
        attackDamage = 3;
        if (ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.DAMAGE)) {
            attackDamage = 21;
        } else if (ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.DAMAGE1)) {
            attackDamage = 61;
        } else if (ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.DAMAGE2)) {
            attackDamage = 121;
        }
        Map<Integer, Integer> enchantmentMap = new HashMap<>();
        if(ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.FORTUNA)){
            enchantmentMap.put(Enchantment.fortune.effectId, 3);
        } else if(ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.FORTUNA1)){
            enchantmentMap.put(Enchantment.fortune.effectId, 5);
        }else if(ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.FORTUNA2)) {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        if (ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.HOE) && !ItemVajraPro.hasUpgrade(stack,EnumUpgradeType.SILKTOUCH)) {
            NBTTagCompound tag = getOrCreateTag(stack);
            tag.setBoolean("isHoe", true);
            ModuleHoe hoe = new ModuleHoe();
            hoe.operationEnergyCost = effPower;
            hoe.onItemUse(stack, player, world, x, y, z, side);
        }
        if(ItemVajraPro.hasUpgrade(stack,EnumUpgradeType.SILKTOUCH) && ItemVajraPro.hasUpgrade(stack, EnumUpgradeType.HOE)){
            NBTTagCompound tag = getOrCreateTag(stack);
    ...
    public Map<Block, Integer> getObliterationList(ItemStack stack) {
        ...
        NBTTagCompound compound = ConfigVajraPro.getCompound(stack);
    ...
    public boolean breakAOEBlocks(ItemStack stack, int x, int y, int z, int breakRadius, int breakDepth, EntityPlayer player) {
        Map<Block, Integer> blockMap = ConfigVajraPro.getBoolean(stack, "ToolVoidJunk", false) ? this.getObliterationList(stack) : new HashMap<>();
        ...
        if (effective) {
            ...
            MovingObjectPosition mop = ConfigVajraPro.raytraceFromEntity(player.worldObj, player, 4.5);
            if (mop == null) {
                ConfigVajraPro.updateGhostBlocks(player, player.worldObj);
            } else {
                ...
                int yPos;
                int zPos;
                if (ConfigVajraPro.getBoolean(stack, "BaseSafeAOE", false)) {
                ...
                for (Object o : items) {
                    ...
                    if (!player.worldObj.isRemote) {
                        ...
                        if (ConfigVajraPro.rapidlyDespawnMinedItems) {
                            item.lifespan = 100;
        ...
        if (tag.getBoolean("silktouch")) {
            try {
                ...
                if (block != Blocks.bedrock && block != Blocks.mob_spawner && block.canHarvestBlock(entityplayer, metaData) && block.getItemDropped(metaData, world.rand, 1) != null) {
                    ...
                    if (QuantumUpgrade.isSimulating()) {
                        ...
                        if (block.canSilkHarvest(world, entityplayer, i, j, k, metaData)) {
                            ...
                            while(i$.hasNext()) {
                                ...
                                ItemVajraPro.dropAsEntity(world, i, j, k, is);
                            }
                ...
                QuantumUpgrade.addLog("Vajra: Error in destroy function (" + var22.getLocalizedMessage() + ")");
            }
        }
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List info, boolean b) {
        ...
        if((ItemVajraPro.hasUpgrade(itemStack,EnumUpgradeType.DEPTH)) || (ItemVajraPro.hasUpgrade(itemStack,EnumUpgradeType.DEPTH1)) || (ItemVajraPro.hasUpgrade(itemStack,EnumUpgradeType.DEPTH2))){
        ...
        if(ItemVajraPro.hasUpgrade(itemStack,EnumUpgradeType.SILKTOUCH)){
        ...
        if((ItemVajraPro.hasUpgrade(itemStack, EnumUpgradeType.AUTOMELTING))){
        ...
        if((ItemVajraPro.hasUpgrade(itemStack, EnumUpgradeType.HOE))){
